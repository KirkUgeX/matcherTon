...
export const SecondBestViewer = () => {
    ...
    const getBestPlayerInfo = async () => {
        ...
        try {
            const humans = await humansStats({ pageNumber: 0, size: 2, sorting });
            const winner = humans.results[1];
    ...
    const goToBestPlayer = () => {
        navigate(ROUTES.TO_BEST_PLAYER(query.id));
    };

    return (
        <StatsPage
            title={t(ln, "second_place_among_the_spectators")}
            ...
            adminButton={
                <Button
                    text={<span>{t(ln, "the_winner_among_the_spectators")} <img src="/images/dropdown-arrow.svg" className="next-step__image"/></span>}
                    onClick={goToBestPlayer}
