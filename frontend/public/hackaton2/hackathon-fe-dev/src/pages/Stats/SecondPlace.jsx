...
export const SecondPlace = () => {
    ...
    const getTableData = async () => {
        let player;
        try {
            player = await getPlayerStats(2, query.id, event.id);
        } catch (e) {
            if (e?.status === 403) {
    ...
    const goToFirstPlace = () => {
        navigate(ROUTES.TO_FIRST_PLACE(query.id));
    };
    ...
    return (
        <Fragment>
            <StatsPage
                ...
                adminButton={
                    <Button
                        text={<span>{t(ln, "first_place")} <img src="/images/dropdown-arrow.svg" className="next-step__image"/></span>}
                        onClick={goToFirstPlace}
